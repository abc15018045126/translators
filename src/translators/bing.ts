...
class BingTranslator {
    ...
    private inflightDetect = new Map<string, Promise<string | undefined>>();
    ...
    parseTranslateResult(result: any, extras: TranslationResult) {
        const parsed = extras || {} as TranslationResult;
    ...
    async detect(text: string) {
        if (!text || !text.trim()) return Promise.resolve(undefined);
        const normalizedText = text.trim().toLowerCase();
        
        if (this.inflightDetect.has(normalizedText)) {
            return this.inflightDetect.get(normalizedText);
        }
        ...
        this.inflightDetect.set(normalizedText, detectPromise);
        return detectPromise.finally(() => this.inflightDetect.delete(normalizedText));
    }
    ...
    async translate(text: string, from: string, to: string, _internalNoChunkFallback = false): Promise<TranslationResult> {
        ...
        if (this.inflightTranslate.has(cacheKey)) {
            return this.inflightTranslate.get(cacheKey)!;
        }
    ...
    // No length-based trigger; try direct call first and only then fallback to adaptive segmentation
        ...
        this.inflightTranslate.set(cacheKey, translatePromise);
        return translatePromise.finally(() => this.inflightTranslate.delete(cacheKey));
    }
