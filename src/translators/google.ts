...
class GoogleTranslator {
     ...
     */
    fallbackExpireTime = 0;

    get isFallback() {
        return Date.now() < this.fallbackExpireTime;
    }
    ...
    /**
     * Audio instance (undefined in Service Worker context).
     */
    AUDIO: any = typeof Audio !== "undefined" ? new Audio() : null;
    ...
    private inflightDetect = new Map<string, Promise<string>>();
    private inflightTranslate = new Map<string, Promise<TranslationResult>>();
    
    // Concurrency lock for updateTKK
    private updateTkkPromise: Promise<void> | null = null;
     ...
     * @returns request TK
     */
    generateTK(a: string | number, b: string | number, c: string | number): string {
        let numB = Number(b) || 0;
        let e: number[] = [];
        let f = 0;
        let g = 0;
        ...
        for (; g < strA.length; g++) {
            ...
            128 > l
                ...
                : (2048 > l
                      ...
                      : (55296 == (l & 64512) &&
                        g + 1 < strA.length &&
                        56320 == (strA.charCodeAt(g + 1) & 64512)
                            ? ((l = 65536 + ((l & 1023) << 10) + (strA.charCodeAt(++g) & 1023)),
                              (e[f++] = (l >> 18) | 240),
                              (e[f++] = ((l >> 12) & 63) | 128))
                            : (e[f++] = (l >> 12) | 224),
                        (e[f++] = ((l >> 6) & 63) | 128)),
        ...
        for (f = 0; f < e.length; f++) {
            currentA += e[f];
            currentA = this._magic(currentA, "+-a^+6");
        }
        currentA = this._magic(currentA, "+-3^+b+-f");
        currentA ^= Number(c) || 0;
        0 > currentA && (currentA = (currentA & 2147483647) + 2147483648);
        currentA %= 1e6;
        return currentA.toString() + "." + (currentA ^ numB);
    }
     ...
     */
    _magic(a: number, b: string): number {
        for (let c = 0; c < b.length - 2; c += 3) {
            let charD = b.charAt(c + 2);
            let d = "a" <= charD ? charD.charCodeAt(0) - 87 : Number(charD);
     ...
     */
    async updateTKK() {
        if (this.updateTkkPromise) return this.updateTkkPromise;

        this.updateTkkPromise = (async () => {
            try {
                ...
                this.updateTkkPromise = null;
            }
        })();
        
        return this.updateTkkPromise;
    }
     ...
     */
    fallBack() {
        this.fallbackExpireTime = Date.now() + 30 * 60 * 1000; // 30 minutes
    }
    ...
    generateDetectURL(text: string) {
        ...
        if (this.isFallback) return this.FALLBACK_TRANSLATE_URL + query;
    ...
    parseTranslateResult(response: any) {
        if (this.isFallback) return this.parseFallbackResult(response);
    ...
    detect(text: string) {
        ...
        const key = `Gd|${fnv1a32(normalizedText)}`;
        ...
        const detectOnce = async (): Promise<string> => {
             ...
             */
            if (response.status === 429 && !this.isFallback) {
                this.fallBack();
        ...
        const exec = detectOnce();
        this.inflightDetect.set(key, exec);
        return exec.finally(() => this.inflightDetect.delete(key));
    }
    ...
    translate(text: string, from: string, to: string) {
        ...
        const normalizedText = text.trim().toLowerCase();
        const key = `Gt|${from}|${to}|${fnv1a32(normalizedText)}`;
        const hit = this.cache.get(key);
        if (hit) return Promise.resolve(hit);

        if (this.inflightTranslate.has(key)) return this.inflightTranslate.get(key)!;
        ...
        const exec = translateOnce();
        this.inflightTranslate.set(key, exec);
        return exec.finally(() => this.inflightTranslate.delete(key));
    }
    ...
    async pronounce(text: string, language: string, speed: PronunciationSpeed) {
        ...
        if (!this.AUDIO) {
            // Can't play audio directly in Service Worker context, typically handled by content script.
            return;
        }

        this.AUDIO.src = `${
            this.isFallback ? this.FALLBACK_TTS_URL : this.TTS_URL
